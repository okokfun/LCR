...
static const uint8_t num_lptia_gain_factors = sizeof(
                        lptia_gain_factors)
...
static const uint8_t num_pga_gain_factors = sizeof(
                        pga_gain_factors)
...
void ad5940_write_reg(ad5940_t* a, ad5940_reg_t reg,
                      uint32_t val) {
...
uint32_t ad5940_read_reg(ad5940_t* a, ad5940_reg_t reg) {
    ...
    uint32_t ret = 0;
    {
        ...
        uint8_t rec[6];
        HAL_SPI_TransmitReceive(a->spi, send, rec, reglength + 2,
                                100);
        if (reglength == 2)
...
void ad5940_set_bits(ad5940_t* a, ad5940_reg_t reg,
                     uint32_t bits) {
...
void ad5940_clear_bits(ad5940_t* a, ad5940_reg_t reg,
                       uint32_t bits) {
    uint32_t val = ad5940_read_reg(a, reg);
    val &= ~bits;
    ad5940_write_reg(a, reg, val);
...
void ad5940_modify_reg(ad5940_t* a, ad5940_reg_t reg,
                       uint32_t val,
                       uint32_t mask) {
...
#ifdef AD5940_USE_MUTEX
    a->access_mutex = xSemaphoreCreateMutexStatic(
...
#error GPIO_SPEED_HIGH not defined, probably slightly different macro name for this mcu
    ...
    for (uint8_t i = 0;
         i < sizeof(RegTable) / sizeof(RegTable[0]); i++)
        ad5940_write_reg(a, RegTable[i].reg_addr,
                         RegTable[i].reg_data);
...
ad5940_result_t ad5940_set_electrode(ad5940_t* a,
                                     ad5940_electrode_mode_t e) {
    switch (e) {
...
ad5940_result_t ad5940_set_vbias(ad5940_t* a, int16_t mV) {
    ...
    int32_t val = ((int32_t)(dac_voltage - 200) * 1000 + 268) /
                  537;
    if (val < 0 || val > 4095) {
        LOG(Log_AD5940, LevelWarn,
...
ad5940_result_t ad5940_set_TIA_lowpass(ad5940_t* a,
                                       ad5940_tia_rf_t rf) {
    ad5940_modify_reg(a, AD5940_REG_LPTIACON0, rf, 0xE000);
    return AD5940_RES_OK;
}

ad5940_result_t ad5940_set_TIA_load(ad5940_t* a,
                                    ad5940_tia_rl_t rl) {
    ad5940_modify_reg(a, AD5940_REG_LPTIACON0, rl, 0x1C00);
    return AD5940_RES_OK;
}

ad5940_result_t ad5940_set_TIA_gain(ad5940_t* a,
                                    ad5940_tia_gain_t g) {
    ad5940_modify_reg(a, AD5940_REG_LPTIACON0, g, 0x03E0);
    return AD5940_RES_OK;
}

ad5940_result_t ad5940_get_TIA_gain(ad5940_t* a,
                                    ad5940_tia_gain_t* g) {
...
ad5940_tia_gain_t ad5940_value_to_LPTIA_gain(
                uint32_t value) {
    for (uint8_t i = 0; i < num_lptia_gain_factors; i++) {
...
ad5940_result_t ad5940_set_ADC_mux(ad5940_t* a,
                                   ad5940_adc_muxp_t p, ad5940_adc_muxn_t n) {
    ad5940_modify_reg(a, AD5940_REG_ADCCON, p | n, 0x1FFF);
    return AD5940_RES_OK;
}

ad5940_result_t ad5940_set_PGA_gain(ad5940_t* a,
                                    ad5940_pga_gain_t gain) {
    ad5940_modify_reg(a, AD5940_REG_ADCCON, gain, 0x00070000);
    return AD5940_RES_OK;
}

ad5940_result_t ad5940_get_PGA_gain(ad5940_t* a,
                                    ad5940_pga_gain_t* gain) {
...
ad5940_result_t ad5940_zero_ADC(ad5940_t* a) {
    ...
    ad5940_set_ADC_mux(a, AD5940_ADC_MUXP_LPTIAPLP,
                       AD5940_ADC_MUXN_AIN4);
...
ad5940_result_t ad5940_set_SINC2_OSR(ad5940_t* a,
                                     ad5940_sinc2_osr_t osr) {
    ad5940_modify_reg(a, AD5940_REG_ADCFILTERCON,
                      ((uint16_t) osr) << 8, 0x0F00);
    return AD5940_RES_OK;
}
...
int32_t ad5940_raw_ADC_to_current(ad5940_t* a, int16_t raw) {
    ...
    int64_t vol_adc_uv = (int64_t) raw * ADC_REFERENCE_uV /
                         32768;
...
ad5940_result_t ad5940_measure_current(ad5940_t* a,
                                       int32_t* nA) {
    uint32_t sum = 0;
...
ad5940_result_t ad5940_enable_FIFO(ad5940_t* a,
                                   ad5940_fifosrc_t src) {
    uint32_t val = (1 << 11)						// enable FIFO
                   | (((uint32_t) src) << 13);		// select source
    ad5940_disable_FIFO(a);
    ad5940_modify_reg(a, AD5940_REG_CMDDATACON, 0x000006C0,
                      0xFFFFFFE0);
...
static uint16_t readFIFO(ad5940_t* a, uint32_t offset) {
    uint8_t recv[4];
    HAL_SPI_TransmitReceive(a->spi, (uint8_t*) &offset, recv, 4,
                            100);
    return recv[3] | (uint16_t) recv[2] << 8;
}
void ad5940_FIFO_read(ad5940_t* a, uint16_t* dest,
                      uint16_t num) {
    if (!num)
        return;
    if (num < 2) {
        *dest = ad5940_read_reg(a,
                                AD5940_REG_DATAFIFORD) & 0x0000FFFF;
        return;
    }
    cs_low(a);
...
ad5940_result_t ad5940_generate_waveform(ad5940_t* a,
                                         ad5940_waveinfo_t* w) {
    ad5940_take_mutex(a);
    ...
    switch (w->type) {
        case AD5940_WAVE_SINE: {
                ...
                while (w->sine.phaseoffset < 0)
                    w->sine.phaseoffset -= 360000;
                uint32_t phase_reg = (uint64_t) w->sine.phaseoffset *
                                     (1UL << 30)
                                     / 360000;
                ...
                uint16_t hsdaccon;
                uint32_t fullscale;
                uint32_t max_voltage = abs(w->sine.offset) + abs(
                                                       w->sine.amplitude);
                ...
                int16_t offset = (int32_t) w->sine.offset * dac_range /
                                 fullscale;
...
ad5940_result_t ad5940_set_excitation_amplifier(ad5940_t* a,
    ...
    ad5940_modify_reg(a, AD5940_REG_SWCON, dsw | psw | nsw,
                      0x0FFF);
    if (dc_bias) {
...
ad5940_result_t ad5940_disable_excitation_amplifier(
                ad5940_t* a) {
    ad5940_take_mutex(a);
...
ad5940_result_t ad5940_set_HSTIA(ad5940_t* a,
                                 ad5940_hsrtia_t rtia,
                                 uint8_t ctia, ad5940_HSTIA_VBIAS_t bias, bool diode,
                                 ad5940_hstsw_t sw) {
    ad5940_take_mutex(a);
...
ad5940_result_t ad5940_set_dft(ad5940_t* a,
    ...
    if (dft->source != AD5940_DFTSRC_DISABLED) {
        ...
        switch (dft->source) {
            ...
            case AD5940_DFTSRC_RAW:
                ...
                ad5940_modify_reg(a, AD5940_REG_DFTCON, (0x02 << 20),
                                  (0x03 << 20));
                break;
            case AD5940_DFTSRC_SINC2:
                ad5940_clear_bits(a, AD5940_REG_ADCFILTERCON, (1UL << 7));
                ad5940_modify_reg(a, AD5940_REG_DFTCON, (0x00 << 20),
                                  (0x03 << 20));
                break;
            case AD5940_DFTSRC_SINC3:
                ad5940_clear_bits(a, AD5940_REG_ADCFILTERCON, (1UL << 7));
                ad5940_modify_reg(a, AD5940_REG_DFTCON, (0x01 << 20),
                                  (0x03 << 20));
                break;
            default:
                break;
        ...
        if (dftnumval > 0x0C) {
            ...
            ad5940_modify_reg(a, AD5940_REG_DFTCON, dftnumval << 4,
                              (0x0F << 4));
            res = AD5940_RES_OK;
        }
...
ad5940_result_t ad5940_get_dft_result(ad5940_t* a,
                                      uint8_t avg, ad5940_dftresult_t* data) {
    ad5940_take_mutex(a);
    data->mag = 0.0f;
    data->phase = 0.0f;
    for (uint8_t i = 0; i < avg; i++) {
        ...
        int32_t real = util_sign_extend_32(ad5940_read_reg(a,
                                           AD5940_REG_DFTREAL), 18);
        int32_t imag = util_sign_extend_32(ad5940_read_reg(a,
                                           AD5940_REG_DFTIMAG), 18);
        LOG(Log_AD5940, LevelDebug, "DFT raw R: %ld, I: %ld", real,
            imag);
...
ad5940_result_t ad5940_setup_four_wire(ad5940_t* a,
                                       uint32_t frequency,
...
#define EX_AMP_MAX_AMPLITUDE		800
    ...
    if (EX_AMP_MAX_AMPLITUDE / (rseries + 1000000000UL / nS_max)
        >= 3) {
    ...
    uint32_t max_voltage = max_current * 1000ULL /
                           nS_min; // in mV
    ...
    ad5940_set_excitation_amplifier(a, amp,
...
ad5940_result_t ad5940_measure_four_wire(ad5940_t* a,
                                         ad5940_adc_muxp_t p_sense,
                                         ad5940_adc_muxn_t n_sense, uint8_t avg,
                                         ad5940_dftresult_t* data) {
    ad5940_take_mutex(a);
    ad5940_result_t res = AD5940_RES_OK;
    ...
    res |= ad5940_set_ADC_mux(a, AD5940_ADC_MUXP_HSTIAP,
...
ad5940_result_t ad5940_disable_HS_loop(ad5940_t* a) {
    ad5940_result_t res = AD5940_RES_OK;
    ad5940_take_mutex(a);
    ad5940_clear_bits(a, AD5940_REG_SWCON,
                      (1UL << 16)); // control switches as group
...
ad5940_result_t ad5940_gpio_configure(ad5940_t* a,
                                      uint8_t gpio, ad5940_gpio_config_t config) {
    ad5940_take_mutex(a);
...
ad5940_result_t ad5940_gpio_clear(ad5940_t* a,
                                  uint8_t gpio) {
    ad5940_take_mutex(a);
