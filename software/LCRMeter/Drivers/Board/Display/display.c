...
void setXY(uint16_t x0, uint16_t y0, uint16_t x1,
           uint16_t y1) {
...
void display_HorizontalLine(int16_t x, int16_t y,
                            uint16_t length) {
    if (y >= active.minY && y <= active.maxY && x <= active.maxX
        && x + length > active.minX) {
        if (x < active.minX) {
...
void display_VerticalLine(int16_t x, int16_t y,
                          uint16_t length) {
    if (x >= active.minX && x <= active.maxX && y <= active.maxY
        && y + length > active.minY) {
        if (y < active.minY) {
...
void display_Line(int16_t x0, int16_t y0, int16_t x1,
                  int16_t y1) {
...
void display_Rectangle(int16_t x0, int16_t y0, int16_t x1,
                       int16_t y1) {
...
void display_RectangleFull(int16_t x0, int16_t y0,
                           int16_t x1, int16_t y1) {
    if (x0 > active.maxX || y0 > active.maxY || x1 < active.minX
        || y1 < active.minY) {
...
void display_Circle(int16_t x0, int16_t y0,
                    uint16_t radius) {
    int x = radius;
    int y = 0;
    int err = 0;
...
void display_CircleFull(int16_t x0, int16_t y0,
                        uint16_t radius) {
...
void display_Char(int16_t x, int16_t y, uint8_t c) {
    if (x > active.maxX || y > active.maxY
        || x + font.width < active.minX
        || y + font.height < active.minY) {
        /* Character completely out of active area, skip */
        return;
    }
    uint16_t skipLeft = 0, skipRight = 0, skipTop = 0,
             skipBottom = 0;
    if (x < active.minX)
    ...
    setXY(x + skipLeft, y + skipTop,
          x + font.width - 1 - skipRight,
          y + font.height - 1 - skipBottom);
    /* number of bytes in font per row */
    uint8_t yInc = (font.width - 1) / 8 + 1;
    const uint8_t* charIndex = font.data + c * yInc *
                               font.height;
    uint8_t i, j;
...
void display_CharRotated(int16_t x, int16_t y, uint8_t c) {
    if (x > active.maxX || y > active.maxY
        || x + font.height < active.minX
        || y + font.width < active.minY) {
        /* Character completely out of active area, skip */
        return;
    }
    uint16_t skipLeft = 0, skipRight = 0, skipTop = 0,
    ...
    setXY(x + skipLeft, y + skipTop,
          x + font.height - 1 - skipRight,
          y + font.width - 1 - skipBottom);
    /* number of bytes in font per row */
    uint8_t yInc = (font.width - 1) / 8 + 1;
    const uint8_t* charIndex = font.data + c * yInc *
                               font.height;
...
void display_StringRotated(int16_t x, int16_t y,
                           const char* s) {
    while (*s) {
...
void display_AutoCenterString(const char* s,
                              coords_t upperLeft,
                              coords_t lowerRight) {
    uint8_t lines = 1;
    ...
    do {
        ...
        display_String(upperLeft.x + shiftX, upperLeft.y + shiftY,
                       strbuf);
        start = linebreak + 1;
        shiftY += font.height;
    } while (linebreak);
...
void display_ImageGrayscale(int16_t x, int16_t y,
                            const Image_t* im) {
    //	usb_DisplayCommand(0, x);
    //	usb_DisplayCommand(1, y);
    //	usb_DisplayCommand(2, x + im->width - 1);
    ...
    for (; i > 0; i--) {
        ...
        uint16_t gray = COLOR_R(*ptr) + COLOR_G(*ptr) + COLOR_B(
                                        *ptr);
        gray /= 3;
...
void display_SetActiveArea(uint16_t minx, uint16_t maxx,
                           uint16_t miny,
                           uint16_t maxy) {
    active.minX = minx;
    active.maxX = maxx;
