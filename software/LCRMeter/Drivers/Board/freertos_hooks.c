...
void configureTimerForRunTimeStats(void) {
    ...
    uint32_t timerFreq = APB1_freq == AHB_freq ? APB1_freq :
                         APB1_freq * 2;
...
void vPortSuppressTicksAndSleep(TickType_t
    ...
    if (pd_sleep) {
        ...
        uint32_t ulReloadValue, ulCompleteTickPeriods,
                 ulCompletedSysTickDecrements, ulSysTickCTRL;
...
int freertos_print_task_overview() {
    ...
    TaskStatus_t* task_states = pvPortMalloc(nTasks * sizeof(
            TaskStatus_t));
    if (!task_states) {
    ...
    nTasks = uxTaskGetSystemState(task_states, nTasks,
                                  &total_runtime);
