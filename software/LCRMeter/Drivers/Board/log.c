...
#endif
    written = snprintf(&fifo[fifo_write], MAX_LINE_LENGTH,
                       "%05lu [%6.6s,%s]: ",
                       HAL_GetTick(), module + 4, lvl_strings[lvl]);
    written += vsnprintf(&fifo[fifo_write + written],
                         MAX_LINE_LENGTH - written,
                         fmt, args);
    written += snprintf(&fifo[fifo_write + written],
                        MAX_LINE_LENGTH - written,
                        "\r\n");
...
#endif
    int16_t overflow = (fifo_write + written) -
                       LOG_SENDBUF_LENGTH;
    if (overflow > 0) {
...
void log_force(const char* fmt, ...) {
    ...
    va_list args;
    va_start(args, fmt);
    written += vsnprintf(&fifo[written],
                         MAX_LINE_LENGTH - written,
                         fmt, args);
    written += snprintf(&fifo[written],
                        MAX_LINE_LENGTH - written,
                        "\r\n");
