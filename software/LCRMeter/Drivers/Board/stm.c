...
int stm_set_rtc(uint32_t timestamp) {
    ...
    if (timeinfo) {
        ...
        date.Year = timeinfo->tm_year -
                    100; // convert from timeinfo offset (1900) to RTC offset (2000)
        date.Month = timeinfo->tm_mon + 1;
        date.Date = timeinfo->tm_mday;
        date.WeekDay = timeinfo->tm_wday + 1;
        LOG(Log_App, LevelDebug, "RTC set: %d:%d:%d %d/%d/%d",
            time.Hours,
            time.Minutes, time.Seconds, date.Date, date.Month,
            date.Year);
...
uint32_t stm_get_rtc() {
    ...
    LOG(Log_App, LevelDebug, "RTC get: %d:%d:%d %d/%d/%d",
        time.Hours,
        time.Minutes, time.Seconds, date.Date, date.Month,
        date.Year);
    return unixtime((uint16_t) date.Year + 2000, date.Month,
