...
PRIVILEGED_DATA static volatile BaseType_t xSchedulerRunning
    = pdFALSE;
PRIVILEGED_DATA static volatile UBaseType_t uxPendedTicks
    = (UBaseType_t) 0U;
PRIVILEGED_DATA static volatile BaseType_t xYieldPending 			=
                pdFALSE;
...
xNextTaskUnblockTime		= (TickType_t)
...
TaskHandle_t xTaskCreateStatic(TaskFunction_t pxTaskCode,
    ...
    if ((pxTaskBuffer != NULL) && (puxStackBuffer != NULL)) {
        ...
        prvInitialiseNewTask(pxTaskCode, pcName, ulStackDepth,
                             pvParameters, uxPriority, &xReturn, pxNewTCB, NULL);
...
BaseType_t xTaskCreate(TaskFunction_t pxTaskCode,
...
#else /* portSTACK_GROWTH */
    {
        StackType_t* pxStack;
        /* Allocate space for the stack used by the task being created. */
        pxStack = (StackType_t*) pvPortMalloc((((size_t)
                                                usStackDepth) * sizeof(
                                                        StackType_t)));      /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
        if (pxStack != NULL) {
...
TaskHandle_t xTaskGetHandle(const char*
    ...
    configASSERT(strlen(pcNameToQuery) <
    ...
    vTaskSuspendAll();
    {
        ...
        if (pxTCB == NULL)
            pxTCB = prvSearchForNameWithinSingleList((List_t*)
                                                     pxDelayedTaskList, pcNameToQuery);
        if (pxTCB == NULL)
            pxTCB = prvSearchForNameWithinSingleList((List_t*)
                                                     pxOverflowDelayedTaskList, pcNameToQuery);
#if ( INCLUDE_vTaskSuspend == 1 )
        {
            if (pxTCB == NULL) {
...
UBaseType_t uxTaskGetSystemState(TaskStatus_t* const
                                 pxTaskStatusArray, const UBaseType_t uxArraySize,
    ...
    vTaskSuspendAll();
    {
        ...
        if (uxArraySize >= uxCurrentNumberOfTasks) {
            ...
            uxTask += prvListTasksWithinSingleList(&
                                                   (pxTaskStatusArray[ uxTask ]), (List_t*) pxDelayedTaskList,
                                                   eBlocked);
            uxTask += prvListTasksWithinSingleList(&
                                                   (pxTaskStatusArray[ uxTask ]),
...
void vTaskStepTick(const TickType_t xTicksToJump) {
    ...
    each stepped tick. */
    configASSERT((xTickCount + xTicksToJump) <=
...
TaskHookFunction_t xTaskGetApplicationTaskTag(
                TaskHandle_t xTask) {
    TCB_t* xTCB;
...
void vTaskPlaceOnEventListRestricted(List_t* const
                                     pxEventList, TickType_t xTicksToWait,
...
BaseType_t xTaskRemoveFromUnorderedEventList(
        ...
        if (pxCurrentTCB->ucDelayAborted != pdFALSE) {
            ...
            if (*pxTicksToWait == portMAX_DELAY) {
                ...
                if ((xNumOfOverflows != pxTimeOut->xOverflowCount)
                    ...
                    was called. */
                    xReturn = pdTRUE;
                } else if (((TickType_t)(xConstTickCount -
...
void vTaskSetThreadLocalStoragePointer(TaskHandle_t
                                       xTaskToSet, BaseType_t xIndex, void* pvValue) {
    TCB_t* pxTCB;
...
void* pvTaskGetThreadLocalStoragePointer(TaskHandle_t
                                         xTaskToQuery, BaseType_t xIndex) {
    void* pvReturn = NULL;
    TCB_t* pxTCB;
    if (xIndex < configNUM_THREAD_LOCAL_STORAGE_POINTERS) {
        pxTCB = prvGetTCBFromHandle(xTaskToQuery);
...
void vTaskGetInfo(TaskHandle_t xTask,
    ...
    pxTaskStatus->pcTaskName = (const char*) &
                               (pxTCB->pcTaskName [ 0 ]);
    ...
    if (listCURRENT_LIST_LENGTH(pxList) > (UBaseType_t) 0) {
        ...
        do {
            ...
            vTaskGetInfo((TaskHandle_t) pxNextTCB,
                         &(pxTaskStatusArray[ uxTask ]), pdTRUE, eState);
            uxTask++;
        } while (pxNextTCB != pxFirstTCB);
    } else
        mtCOVERAGE_TEST_MARKER();
...
#endif
    uxReturn = (UBaseType_t) prvTaskCheckFreeStackSpace(
                               pucEndOfStack);
    return uxReturn;
}
...
static char* prvWriteNameToBuffer(char* pcBuffer,
    ...
    for (x = strlen(pcBuffer);
         x < (size_t)(configMAX_TASK_NAME_LEN - 1); x++)
        pcBuffer[ x ] = ' ';
    /* Terminate. */
    pcBuffer[ x ] = 0x00;
...
BaseType_t xTaskGenericNotifyFromISR(TaskHandle_t
