...
static TickType_t xCoRoutineTickCount = 0,
                  xLastTickCount = 0, xPassedTicks = 0;
...
static void prvInitialiseCoRoutineLists(void) {
    ...
    for (uxPriority = 0;
         uxPriority < configMAX_CO_ROUTINE_PRIORITIES; uxPriority++)
        vListInitialise((List_t*) &
...
BaseType_t xCoRoutineRemoveFromEventList(const List_t*
    ...
    if (pxUnblockedCRCB->uxPriority >=
        pxCurrentCoRoutine->uxPriority)
        xReturn = pdTRUE;
    else
        xReturn = pdFALSE;
    return xReturn;
