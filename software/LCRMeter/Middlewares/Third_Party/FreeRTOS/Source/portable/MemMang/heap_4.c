 ...
 */
static void prvInsertBlockIntoFreeList(BlockLink_t*
                                       pxBlockToInsert);
...
static const size_t xHeapStructSize	= (sizeof(
        BlockLink_t) + ((size_t)(portBYTE_ALIGNMENT - 1))) & ~((
...
static void prvHeapInit(void) {
    ...
    pxFirstFreeBlock->xBlockSize = uxAddress -
                                   (size_t) pxFirstFreeBlock;
    ...
    xBlockAllocatedBit = ((size_t) 1) << ((sizeof(
            size_t) * heapBITS_PER_BYTE) - 1);
}
/*-----------------------------------------------------------*/
...
static void prvInsertBlockIntoFreeList(BlockLink_t*
    ...
    if ((puc + pxIterator->xBlockSize) == (uint8_t*)
        pxBlockToInsert) {
