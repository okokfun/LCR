 ...
 */
static BaseType_t prvTestWaitCondition(const EventBits_t
                                       uxCurrentEventBits, const EventBits_t uxBitsToWaitFor,
...
EventBits_t xEventGroupSync(EventGroupHandle_t xEventGroup,
    ...
    traceEVENT_GROUP_SYNC_END(xEventGroup, uxBitsToSet,
...
EventBits_t xEventGroupClearBits(EventGroupHandle_t
    ...
    itself. */
    configASSERT(xEventGroup);
    configASSERT((uxBitsToClear & eventEVENT_BITS_CONTROL_BYTES)
                 == 0);
    taskENTER_CRITICAL();
    {
...
BaseType_t xEventGroupClearBitsFromISR(EventGroupHandle_t
                                       xEventGroup, const EventBits_t uxBitsToClear) {
    BaseType_t xReturn;
    traceEVENT_GROUP_CLEAR_BITS_FROM_ISR(xEventGroup,
                                         uxBitsToClear);
...
EventBits_t xEventGroupSetBits(EventGroupHandle_t
    ...
    List_t* pxList;
    EventBits_t uxBitsToClear = 0, uxBitsWaitedFor,
                uxControlBits;
    EventGroup_t* pxEventBits = (EventGroup_t*) xEventGroup;
    ...
    itself. */
    configASSERT(xEventGroup);
    configASSERT((uxBitsToSet & eventEVENT_BITS_CONTROL_BYTES) ==
                 0);
...
static BaseType_t prvTestWaitCondition(const EventBits_t
    ...
    if (xWaitForAllBits == pdFALSE) {
        ...
        if ((uxCurrentEventBits & uxBitsToWaitFor) !=
            (EventBits_t) 0)
