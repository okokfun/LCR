...
#if _FS_LOCK
    static FILESEM
    Files[_FS_LOCK];	/* Open object lock semaphores */
#endif
...
#if _USE_LFN
static void gen_numname(BYTE* dst, const BYTE* src,
                        const WCHAR* lfn, UINT seq);
#endif /* !_USE_LFN */
...
UINT inc_lock(  /* Increment object open counter and returns its index (0:Internal error) */
    ...
    Files[i].ctr = acc ? 0x100 : Files[i].ctr +
...
FRESULT sync_window(
    ...
    if (fs->wflag) {	/* Write back the sector if it is dirty */
        ...
        if (disk_write(fs->drv, fs->win.d8, wsect, 1) != RES_OK)
            ...
            fs->wflag = 0;
            if (wsect - fs->fatbase <
                fs->fsize) {		/* Is it in the FAT area? */
            ...
            case FS_FAT12 :
                ...
                *p = (clst & 1) ? ((*p & 0x0F) | ((BYTE)val << 4)) :
                ...
                if (res != FR_OK)
                    break;
                p = &fs->win.d8[bc % SS(fs)];
                *p = (clst & 1) ? (BYTE)(val >> 4) : ((*p & 0xF0) | ((BYTE)(
                        val >> 8) & 0x0F));
                fs->wflag = 1;
                break;
            case FS_FAT16 :
...
DWORD create_chain(  /* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
    ...
    if (res == FR_OK && clst != 0) {
        res = put_fat(fs, clst,
                      ncl);	/* Link it to the previous one if needed */
    }
    if (res == FR_OK) {
...
FRESULT dir_sdi(
    ...
    if (clst == 1
        || clst >= dp->fs->n_fatent)	/* Check start cluster range */
        return FR_INT_ERR;
    if (!clst && dp->fs->fs_type ==
    ...
    if (clst ==
        ...
        ic = SS(dp->fs) / SZ_DIRE *
             dp->fs->csize;	/* Entries per cluster */
    ...
    if (!sect)
        return FR_INT_ERR;
    dp->sect = sect + idx / (SS(dp->fs) /
                             SZ_DIRE);					/* Sector# of the directory entry */
    dp->dir = dp->fs->win.d8 + (idx % (SS(dp->fs) / SZ_DIRE)) *
...
FRESULT dir_next(  /* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
        ...
        if (!dp->clust) {		/* Static table */
            ...
            if (((i / (SS(dp->fs) / SZ_DIRE)) & (dp->fs->csize - 1)) ==
                ...
                if (clst >=
                    ...
                    mem_set(dp->fs->win.d8, 0,
                            SS(dp->fs));		/* Clear window buffer */
                    dp->fs->winsect = clust2sect(dp->fs,
                                                 clst);	/* Cluster start sector */
    ...
    dp->dir = dp->fs->win.d8 + (i % (SS(dp->fs) / SZ_DIRE)) *
...
int cmp_lfn(    /* 1:Matched, 0:Not matched */
    ...
    UINT i, s;
    WCHAR wc, uc;
    i = ((dir[LDIR_Ord] & ~LLEF) - 1) *
        13;	/* Get offset in the LFN buffer */
    s = 0;
    wc = 1;
    do {
        uc = LD_WORD(dir +
                     LfnOfs[s]);	/* Pick an LFN character from the entry */
...
int pick_lfn(    /* 1:Succeeded, 0:Buffer overflow */
    ...
    UINT i, s;
    WCHAR wc, uc;
    i = ((dir[LDIR_Ord] & 0x3F) - 1) *
        13;	/* Offset in the LFN buffer */
    s = 0;
    wc = 1;
    do {
        uc = LD_WORD(dir +
...
FRESULT dir_register(  /* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
    ...
    mem_cpy(sn, fn, 12);
    if (_FS_RPATH
        && (sn[NSFLAG] & NS_DOT))		/* Cannot create dot entry */
    ...
    if (sn[NSFLAG] &
        NS_LFN) {			/* When LFN is to be created, allocate entries for an SFN + LFNs. */
        for (n = 0; lfn[n]; n++) ;
        nent = (n + 25) / 13;
    ...
    if (res == FR_OK && --nent) {	/* Set LFN entry if needed */
        ...
        if (res == FR_OK) {
            sum = sum_sfn(
                                  dp->fn);	/* Sum value of the SFN tied to the LFN */
...
#if _USE_LFN
            if (IsUpper(c)
                && (dir[DIR_NTres] & (i >= 9 ? NS_EXT : NS_BODY)))
                c += 0x20;			/* To lower */
#if _LFN_UNICODE
...
WCHAR get_achar(   /* Get a character and advances ptr 1 or 2 */
    ...
    if (IsDBCS1(chr)
        && IsDBCS2(**ptr))		/* Get DBC 2nd byte if needed */
        chr = chr << 8 | (BYTE) * (*ptr)++;
#ifdef _EXCVT
    if (chr >= 0x80)
...
FRESULT create_name(
    ...
    for (p = *path; *p == '/'
         || *p == '\\'; p++) ;	/* Strip duplicated separator */
    lfn = dp->lfn;
    si = di = 0;
    for (;;) {
        ...
        if (w < 0x80
            && chk_chr("\"*:<>\?|\x7F",
                       w)) /* Reject illegal characters for LFN */
            return FR_INVALID_NAME;
    ...
    *path = &p[si];						/* Return pointer to the next segment */
    ...
    for (;;) {
        ...
        if (_DF1S && w >=
            0x100) {		/* DBC (always false at SBCS cfg) */
            if (i >= ni - 1) {
                cf |= NS_LOSS | NS_LFN;
                i = ni;
                continue;
    ...
    if (dp->fn[0] == DDEM)
        dp->fn[0] =
                        RDDEM;	/* If the first character collides with deleted mark, replace it with RDDEM */
    if (ni == 8)
        b <<= 2;
    if ((b & 0x0C) == 0x0C
        || (b & 0x03) ==
...
#if _FS_RPATH
    if (*path == '/'
        || *path == '\\') {	/* There is a heading separator */
        path++;
        dp->sclust =
                        0;				/* Strip it and start from the root directory */
    } else {								/* No heading separator */
        dp->sclust =
                        dp->fs->cdir;			/* Start from the current directory */
    }
#else
    if (*path == '/'
        ...
        path++;
    dp->sclust =
                    0;							/* Always start from the root directory */
#endif
    if ((UINT)*path <
        ...
        for (;;) {
            res = create_name(dp,
                              &path);	/* Get a segment name of the path */
            if (res != FR_OK)
                break;
            res = dir_find(
                                  dp);				/* Find an object with the sagment name */
            ns = dp->fn[NSFLAG];
...
BYTE check_fs(  /* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
    ...
    if (LD_WORD(&fs->win.d8[BS_55AA]) !=
        ...
        return 2;
    if ((LD_DWORD(&fs->win.d8[BS_FilSysType]) & 0xFFFFFF) ==
        0x544146)		/* Check "FAT" string */
        return 0;
    if ((LD_DWORD(&fs->win.d8[BS_FilSysType32]) & 0xFFFFFF) ==
        0x544146)	/* Check "FAT" string */
        return 0;
    return 1;
}
...
FRESULT find_volume(  /* FR_OK(0): successful, !=0: any error occurred */
    ...
    *rfs = fs;							/* Return pointer to the file system object */
    ...
    fs->drv = LD2PD(
                              vol);				/* Bind the logical drive and a physical drive */
    stat = disk_initialize(
                           fs->drv);	/* Initialize the physical drive */
    if (stat&
        STA_NOINIT)				/* Check if the initialization succeeded */
    ...
    if (fmt == 1 || (!fmt
        ...
        i = LD2PT(
                            vol);						/* Partition number: 0:auto, 1-4:forced */
        if (i)
            i--;
        do {								/* Find an FAT volume */
            bsect = br[i];
            fmt = bsect ? check_fs(fs,
                                   bsect) : 2;	/* Check the partition */
    ...
    if (LD_WORD(fs->win.d8 + BPB_BytsPerSec) != SS(
                        fs))	/* (BPB_BytsPerSec must be equal to the physical sector size) */
        return FR_NO_FILESYSTEM;
    fasize = LD_WORD(fs->win.d8 +
                     BPB_FATSz16);			/* Number of sectors per FAT */
    if (!fasize)
        fasize = LD_DWORD(fs->win.d8 + BPB_FATSz32);
    fs->fsize = fasize;
    fs->n_fats =
                    fs->win.d8[BPB_NumFATs];					/* Number of FAT copies */
    if (fs->n_fats != 1
        && fs->n_fats != 2)				/* (Must be 1 or 2) */
        return FR_NO_FILESYSTEM;
    fasize *= fs->n_fats;								/* Number of sectors for FAT area */
    fs->csize =
                    fs->win.d8[BPB_SecPerClus];				/* Number of sectors per cluster */
    if (!fs->csize
        || (fs->csize & (fs->csize - 1)))	/* (Must be power of 2) */
        return FR_NO_FILESYSTEM;
    fs->n_rootdir = LD_WORD(fs->win.d8 +
                            BPB_RootEntCnt);	/* Number of root directory entries */
    if (fs->n_rootdir % (SS(fs) /
                         SZ_DIRE))				/* (Must be sector aligned) */
        return FR_NO_FILESYSTEM;
    tsect = LD_WORD(fs->win.d8 +
                    BPB_TotSec16);			/* Number of sectors on the volume */
    if (!tsect)
        tsect = LD_DWORD(fs->win.d8 + BPB_TotSec32);
    nrsv = LD_WORD(fs->win.d8 +
                   BPB_RsvdSecCnt);			/* Number of reserved sectors */
    if (!nrsv)
        return FR_NO_FILESYSTEM;					/* (Must not be 0) */
    /* Determine the FAT sub type */
    sysect = nrsv + fasize + fs->n_rootdir / (SS(
            fs) / SZ_DIRE);	/* RSV + FAT + DIR */
    if (tsect < sysect)
        return FR_NO_FILESYSTEM;		/* (Invalid volume size) */
    nclst = (tsect - sysect) /
            fs->csize;				/* Number of clusters */
    if (!nclst)
        return FR_NO_FILESYSTEM;				/* (Invalid volume size) */
    fmt = FS_FAT12;
    if (nclst >= MIN_FAT16)
    ...
    if (fs->fsize < (szbfat + (SS(fs) - 1)) / SS(
                        fs))	/* (BPB_FATSz must not be less than the size needed) */
        return FR_NO_FILESYSTEM;
#if !_FS_READONLY
...
#if (_FS_NOFSINFO & 3) != 3
    if (fmt ==
        FS_FAT32				/* Enable FSINFO only if FAT32 and BPB_FSInfo is 1 */
...
FRESULT validate(  /* FR_OK(0): The object is valid, !=0: Invalid */
    ...
    FIL* fil = (FIL*)
               obj;	/* Assuming offset of .fs and .id in the FIL/DIR structure is identical */
    if (!fil || !fil->fs || !fil->fs->fs_type
        || fil->fs->id != fil->id
...
FRESULT f_open(
    ...
    if (res == FR_OK) {
        ...
        if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS |
            ...
            if (res == FR_OK
                && (mode &
                    FA_CREATE_ALWAYS)) {	/* Truncate it if overwrite mode */
        ...
        if (res == FR_OK) {
            if (mode&
                ...
                mode |= FA__WRITTEN;
            fp->dir_sect =
                            dj.fs->winsect;		/* Pointer to the directory entry */
            fp->dir_ptr = dir;
#if _FS_LOCK
    ...
    *br = 0;	/* Clear read byte counter */
    ...
    for (;  btr;								/* Repeat until all data read */
        ...
        if ((fp->fptr % SS(fp->fs)) ==
            ...
            sect = clust2sect(fp->fs,
                              fp->clust);	/* Get current sector */
            if (!sect)
                ABORT(fp->fs, FR_INT_ERR);
            sect += csect;
            cc = btr / SS(
                                 fp->fs);				/* When remaining bytes >= sector size, */
            if (cc) {							/* Read maximum contiguous sectors directly */
                if (csect + cc >
                    fp->fs->csize)	/* Clip at cluster boundary */
            ...
            if (fp->dsect !=
                ...
                if (fp->flag&
                    ...
                    if (disk_write(fp->fs->drv, fp->buf.d8, fp->dsect,
                                   1) != RES_OK)
                        ABORT(fp->fs, FR_DISK_ERR);
                    fp->flag &= ~FA__DIRTY;
                }
#endif
                if (disk_read(fp->fs->drv, fp->buf.d8, sect,
        ...
        rcnt = SS(fp->fs) - ((UINT)fp->fptr % SS(
                                             fp->fs));	/* Get partial sector data from sector buffer */
        if (rcnt > btr)
            rcnt = btr;
#if _FS_TINY
        if (move_window(fp->fs,
                        fp->dsect) != FR_OK)		/* Move sector window */
            ABORT(fp->fs, FR_DISK_ERR);
        mem_cpy(rbuff, &fp->fs->win.d8[fp->fptr % SS(fp->fs)],
                rcnt);	/* Pick partial sector */
#else
        mem_cpy(rbuff, &fp->buf.d8[fp->fptr % SS(fp->fs)],
                rcnt);	/* Pick partial sector */
#endif
    }
    ...
    *bw = 0;	/* Clear write byte counter */
    ...
    for (;  btw;							/* Repeat until all data written */
        ...
        if ((fp->fptr % SS(fp->fs)) ==
            ...
            if (cc) {						/* Write maximum contiguous sectors directly */
                ...
                if (fp->fs->winsect - sect <
                    ...
                    mem_cpy(fp->fs->win.d8,
                            wbuff + ((fp->fs->winsect - sect) * SS(fp->fs)), SS(fp->fs));
                    fp->fs->wflag = 0;
                }
#else
                if (fp->dsect - sect <
                    cc) { /* Refill sector cache if it gets invalidated by the direct write */
                    mem_cpy(fp->buf.d8,
                            wbuff + ((fp->dsect - sect) * SS(fp->fs)), SS(fp->fs));
                    fp->flag &= ~FA__DIRTY;
                }
#endif
#endif
        ...
        if (move_window(fp->fs,
                        fp->dsect) != FR_OK)	/* Move sector window */
            ABORT(fp->fs, FR_DISK_ERR);
        mem_cpy(&fp->fs->win.d8[fp->fptr % SS(fp->fs)], wbuff,
                wcnt);	/* Fit partial sector */
        fp->fs->wflag = 1;
#else
        mem_cpy(&fp->buf.d8[fp->fptr % SS(fp->fs)], wbuff,
                wcnt);	/* Fit partial sector */
        fp->flag |= FA__DIRTY;
#endif
    }
...
FRESULT f_sync(
    ...
    if (res == FR_OK) {
        if (fp->flag&
            FA__WRITTEN) {	/* Has the file been written? */
...
FRESULT f_getcwd(
    ...
    *buff = 0;
    /* Get logical drive number */
    res = find_volume(&dj.fs, (const TCHAR**)&buff,
                      0);	/* Get current volume */
    if (res == FR_OK) {
        INIT_BUF(dj);
        i = len;			/* Bottom of buffer (directory stack base) */
        dj.sclust =
                        dj.fs->cdir;			/* Start to follow upper directory from current directory */
        while ((ccl = dj.sclust) !=
            ...
            if (res != FR_OK)
                break;
            dj.sclust = ld_clust(dj.fs,
                                 dj.dir);	/* Goto parent directory */
            res = dir_sdi(&dj, 0);
            if (res != FR_OK)
                break;
...
FRESULT f_lseek(
                ...
                do {
                    ...
                    if (ulen <=
                        ...
                        *tbl++ = tcl;
                         ...
                         fp->fs->n_fatent);	/* Repeat until end of chain */
            }
...
#endif
                    if (disk_read(fp->fs->drv, fp->buf.d8, dsc,
        ...
        /* Normal Seek */
    {
        ...
        fp->fptr = nsect = 0;
        if (ofs) {
            bcs = (DWORD)fp->fs->csize * SS(
                                  fp->fs);	/* Cluster size (byte) */
            if (ifptr > 0 &&
                (ofs - 1) / bcs >= (ifptr - 1) /
            ...
            if (clst != 0) {
                while (ofs > bcs) {						/* Cluster following loop */
                    ...
                    if (fp->flag&
                        ...
                        clst = create_chain(fp->fs,
                                            clst);	/* Force stretch if in write mode */
...
#endif
            if (disk_read(fp->fs->drv, fp->buf.d8, nsect,
                          1) != RES_OK)	/* Fill sector cache */
                ABORT(fp->fs, FR_DISK_ERR);
#endif
            fp->dsect = nsect;
...
#if _USE_LFN
        if (fno->lfname
            && pattern_matching(dp->pat, fno->lfname, 0, 0))
...
FRESULT f_truncate(
    ...
    if (res == FR_OK) {
        if (fp->fsize > fp->fptr) {
            fp->fsize =
                            fp->fptr;	/* Set file size to current R/W point */
            fp->flag |= FA__WRITTEN;
            if (fp->fptr ==
    ...
    if (res == FR_OK) {
        ...
        if (res == FR_NO_FILE) {				/* Can create a new directory */
            ...
            if (res ==
                ...
                ST_DWORD(dir + DIR_WrtTime, tm);
                st_clust(dir, dcl);
                mem_cpy(dir + SZ_DIRE, dir,
                        SZ_DIRE); 	/* Create ".." entry */
                dir[SZ_DIRE + 1] = '.';
                pcl = dj.sclust;
            ...
            if (res != FR_OK) {
                remove_chain(dj.fs,
                             dcl);			/* Could not register, remove cluster chain */
            } else {
                dir = dj.dir;
    ...
    if (res == FR_OK) {
        ...
        if (res == FR_OK) {
            ...
            if (!dir) 						/* Is it a root directory? */
                ...
                dir[DIR_Attr] = (attr & mask) | (dir[DIR_Attr] &
...
FRESULT f_rename(
                const TCHAR*
                path_old,	/* Pointer to the object to be renamed */
        ...
        if (res == FR_OK) {						/* Old object is found */
            if (!djo.dir) 						/* Is root dir? */
                ...
                if (get_ldnumber(&path_new) >=
                    ...
                    res = follow_path(&djn,
                                      path_new);	/* and make sure if new object name is not conflicting */
                else
                    res = FR_INVALID_DRIVE;
                if (res == FR_OK)
                ...
                if (res ==
                    ...
                    if (res == FR_OK) {
                        ...
                        if ((dir[DIR_Attr] & AM_DIR)
                            && djo.sclust !=
                            djn.sclust) {	/* Update .. entry in the sub-directory if needed */
...
FRESULT f_forward(
    ...
    *bf = 0;	/* Clear transfer byte counter */
    ...
    for (;  btf
        ...
        sect = clust2sect(fp->fs,
                          fp->clust);		/* Get current data sector */
        if (!sect)
            ABORT(fp->fs, FR_INT_ERR);
        sect += csect;
        if (move_window(fp->fs,
                        sect) != FR_OK)		/* Move sector window */
            ABORT(fp->fs, FR_DISK_ERR);
        fp->dsect = sect;
        ...
        if (rcnt > btf)
            rcnt = btf;
        rcnt = (*func)(&fp->fs->win.d8[(WORD)fp->fptr % SS(fp->fs)],
                       rcnt);
        if (!rcnt)
            ABORT(fp->fs, FR_INT_ERR);
    }
...
FRESULT f_mkfs(
    ...
    part = LD2PT(
                           vol);	/* Partition (0:auto detect, 1-4:get from partition table)*/
    /* Get disk statics */
    ...
    if (fmt == FS_FAT32) {
        ...
        n_fat = (fmt == FS_FAT12) ? (n_clst * 3 + 1) / 2 + 3 :
                (n_clst * 2) + 4;
    ...
    if (_MULTI_PARTITION && part) {
        ...
        tbl = &fs->win.d8[MBR_Table + (part - 1) * SZ_PTE];
        tbl[4] = sys;
        if (disk_write(pdrv, fs->win.d8, 0,
                       1) != RES_OK)	/* Write it to teh MBR */
            return FR_DISK_ERR;
        md = 0xF8;
    } else {
    ...
    mem_set(tbl, 0, SS(fs));
    mem_cpy(tbl, "\xEB\xFE\x90" "MSDOS5.0",
    ...
    if (fmt == FS_FAT32) {
        ...
        ST_DWORD(tbl + BPB_FATSz32,
                 n_fat);	/* Number of sectors per FAT */
        ST_DWORD(tbl + BPB_RootClus,
    ...
    if (disk_write(pdrv, tbl, b_vol,
        ...
        return FR_DISK_ERR;
    if (fmt ==
        FS_FAT32)					/* Write backup VBR if needed (VBR + 6) */
        disk_write(pdrv, tbl, b_vol + 6, 1);
    /* Initialize FAT area */
    wsect = b_fat;
    for (i = 0; i < N_FATS;
         i++) {		/* Initialize each FAT copy */
        ...
        if (disk_write(pdrv, tbl, wsect++, 1) != RES_OK)
            return FR_DISK_ERR;
        mem_set(tbl, 0, SS(
                                fs));			/* Fill following FAT entries with zero */
        for (n = 1; n < n_fat;
    ...
    {
        eb[0] = wsect;
        eb[1] = wsect + (n_clst - ((fmt == FS_FAT32) ? 1 : 0)) * au -
                1;
    ...
    if (fmt == FS_FAT32) {
        ...
        ST_DWORD(tbl + FSI_Nxt_Free,
                 2);			/* Last allocated cluster# */
        ST_WORD(tbl + BS_55AA, 0xAA55);
        disk_write(pdrv, tbl, b_vol + 1,
                   1);	/* Write original (VBR + 1) */
        disk_write(pdrv, tbl, b_vol + 7,
                   1);	/* Write backup (VBR + 7) */
    }
    return (disk_ioctl(pdrv, CTRL_SYNC,
                       0) == RES_OK) ? FR_OK : FR_DISK_ERR;
}
