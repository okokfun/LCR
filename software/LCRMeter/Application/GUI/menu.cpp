...
void Menu::draw(coords_t offset) {
    ...
    uint8_t pageOffset = (uint8_t)(selectedEntry /
                                   entriesPerPage)
                         * entriesPerPage;
    ...
    if (usePages) {
        ...
        display_HorizontalLine(upperLeft.x + 1,
                               upperLeft.y + entriesPerPage * EntrySizeY,
                               size.x - 1);
        display_HorizontalLine(upperLeft.x + 1,
        ...
        display_HorizontalLine(upperLeft.x + 1,
                               ...
                               size.x - 1);
        display_VerticalLine(upperLeft.x,
                             upperLeft.y + entriesPerPage * EntrySizeY + 1,
                             EntrySizeY - 2);
        display_VerticalLine(upperLeft.x + 1,
                             upperLeft.y + entriesPerPage * EntrySizeY + 1,
                             EntrySizeY - 2);
        ...
        page[3] = 0;
        uint16_t shiftX = (size.x - strlen(page) * Font_Big.width) /
                          2;
...
void Menu::drawChildren(coords_t offset) {
    ...
    uint8_t pageOffset = (uint8_t)(selectedEntry /
                                   entriesPerPage)
                         * entriesPerPage;
    MenuEntry* entry = static_cast<MenuEntry*>
                       (firstChild->GetNth(pageOffset));
    for (uint8_t i = 0; i < entriesPerPage; i++) {
        if (i + pageOffset >= nentries)
            break;
        if (entry->getType() == Widget::Type::Menu) {
            ...
            display_SetForeground(Foreground);
            display_AutoCenterString(submenu->name, upperLeft,
                                     lowerRight);
        } else
            Widget::draw(entry, COORDS(offset.x,
                                       offset.y + i * EntrySizeY));
        entry = static_cast<MenuEntry*>(entry->next);
    }
}
...
void Menu::input(GUIEvent_t* ev) {
    ...
    if (ev->type != EVENT_NONE) {
        ...
        if (ev->type == EVENT_TOUCH_DRAGGED
            ...
            ev->pos.x -= 2;
            ev->pos.y -= 2 + (selectedEntry % entriesPerPage) *
                         EntrySizeY;
            ;
        }
