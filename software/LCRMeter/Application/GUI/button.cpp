...
void Button::draw(coords_t offset) {
    ...
    display_VerticalLine(upperLeft.x, upperLeft.y + 1,
                         size.y - 2);
    display_VerticalLine(lowerRight.x, upperLeft.y + 1,
                         size.y - 2);
    display_HorizontalLine(upperLeft.x + 1, upperLeft.y,
                           size.x - 2);
    display_HorizontalLine(upperLeft.x + 1, lowerRight.y,
                           size.x - 2);
    if (selectable) {
        if (!pressed)
            display_SetForeground(color_Tint(Foreground, Background,
                                             75));
        else
            display_SetForeground(color_Tint(Foreground, COLOR_WHITE,
                                             200));
    }
    display_VerticalLine(lowerRight.x - 1, upperLeft.y + 1,
                         size.y - 2);
    ...
    display_VerticalLine(upperLeft.x + 1, upperLeft.y + 1,
                         size.y - 3);
    ...
    if (name) {
        ...
        display_String(upperLeft.x + fontStart.x,
                       upperLeft.y + fontStart.y,
                       name);
    }
