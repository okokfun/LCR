...
Checkbox::Checkbox(bool* value, Callback cb, void* ptr,
                   coords_t size) {
    this->value = value;
    this->cb = cb;
    cbptr = ptr;
...
void Checkbox::draw(coords_t offset) {
    ...
    if (*value) {
        ...
        display_Line(upperLeft.x + 3, upperLeft.y + 3,
                     lowerRight.x - 3,
                     lowerRight.y - 3);
        display_Line(upperLeft.x + 4, upperLeft.y + 3,
                     lowerRight.x - 3,
                     lowerRight.y - 4);
        display_Line(upperLeft.x + 3, upperLeft.y + 4,
                     lowerRight.x - 4,
                     lowerRight.y - 3);
        display_Line(upperLeft.x + 3, lowerRight.y - 3,
                     lowerRight.x - 3,
                     upperLeft.y + 3);
        display_Line(upperLeft.x + 4, lowerRight.y - 3,
                     lowerRight.x - 3,
                     upperLeft.y + 4);
        display_Line(upperLeft.x + 3, lowerRight.y - 4,
                     lowerRight.x - 4,
                     upperLeft.y + 3);
    }
}
