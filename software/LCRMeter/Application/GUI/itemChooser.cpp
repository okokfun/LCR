...
ItemChooser::ItemChooser(const char* const* items,
                         uint8_t* value, font_t font,
...
void ItemChooser::draw(coords_t offset) {
    ...
    for (line = 0; line < lines; line++) {
        ...
        display_String(upperLeft.x + 1,
                       upperLeft.y + 2 + line * font.height,
                       itemlist[index]);
    ...
    /* display scrollbar */
    display_SetForeground(Border);
    display_VerticalLine(lowerRight.x - ScrollbarSize,
                         upperLeft.y, size.y);
    /* calculate beginning and end of scrollbar */
    uint8_t scrollBegin = util_Map(topVisibleEntry, 0, numItems,
                                   0, size.y);
    uint8_t scrollEnd = size.y;
    if (numItems > lines)
        scrollEnd = util_Map(topVisibleEntry + lines, 0, numItems, 0,
                             size.y);
        ...
        case EVENT_TOUCH_DRAGGED:
            if (ev->pos.x >= size.x - ScrollbarSize) {
                ...
                int16_t newOffset = util_Map(ev->pos.y, scrollBarLength / 2,
                                             ...
                                             numItems - lines);
                newOffset = constrain_int16_t(newOffset, 0,
                                              numItems - lines);
