...
Window::Window(const char* title, font_t font,
               coords_t size) {
    this->size = size;
...
void Window::setMainWidget(Widget* w) {
    ...
    if (w->getSize().x > maxSize.x
        || w->getSize().y > maxSize.y) {
...
void Window::draw(coords_t offset) {
    ...
    display_RectangleFull(upperLeft.x + 1/*+ font.height + 4*/,
                          upperLeft.y + 1,
    ...
    display_String(upperLeft.x + 1/*+ font.height + 5*/,
                   upperLeft.y + 2,
                   title);
}
