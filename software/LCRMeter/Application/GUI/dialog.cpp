...
#define INPUT_DIALOG_LENGTH		10
    ...
    Result MessageBox(const char* title, font_t font,
                      const char* msg,
        ...
        switch (type) {
            case MsgBox::OK: {
                    Button* bOK = new Button("OK", Font_Big, MessageBoxButton,
                                             nullptr, COORDS(65, 0));
                    c->attach(bOK, COORDS((c->getSize().x - bOK->getSize().x) /
                                          2,
            ...
            case MsgBox::ABORT_OK: {
                    Button* bOK = new Button("OK", Font_Big, MessageBoxButton,
                                             nullptr, COORDS(65, 0));
                    Button* bAbort = new Button("ABORT", Font_Big,
                                                MessageBoxButton, nullptr, COORDS(65, 0));
                    c->attach(bAbort,
...
#define MAX_NUMBER_OF_FILES		50
        ...
        DIR* dj = (DIR*) pvPortMalloc(sizeof(
                DIR)); /* Directory search object */
        ...
        Container* c = new Container(w->getAvailableArea());
        Button* bAbort = new Button("ABORT", Font_Big, [](void*,
        Widget * w) {
        ...
        if (foundFiles) {
            ...
            Button* bOK = new Button("OK", Font_Big, [](void*,
            Widget * w) {
    ...
    static void stringInputChar(char c) {
        if (c == 0x08) {
            ...
            if (dialog.StringInput.pos > 0) {
                ...
                dialog.StringInput.lString->setText(
                                dialog.StringInput.string);
            }
        } else {
            /* append character if space is available */
            if (dialog.StringInput.pos < dialog.StringInput.maxLength -
                1) {
    ...
    bool StringInput(const char* title, char* result,
                     uint8_t maxLength,
        ...
        /* Create window */
        dialog.StringInput.w = new Window(title, Font_Big,
                                          COORDS(313, 233));
        Container* c = new Container(
                        dialog.StringInput.w->getAvailableArea());
        Keyboard* k = new Keyboard(stringInputChar);
        dialog.StringInput.lString = new Label(
                        c->getSize().x / Font_Big.width, Font_Big,
        ...
        Button* bOK = new Button("OK", Font_Big, [](void*,
        Widget * w) {
            strcpy(dialog.StringInput.destString,
                   dialog.StringInput.string);
    ...
    Result StringInputBlock(const char* title, char* result,
                            uint8_t maxLength) {
        if (xTaskGetCurrentTaskHandle() == GUIHandle) {
            /* This dialog must never be called by the GUI thread (Deadlock) */
            LOG(Log_GUI, LevelCrit, "Dialog started from GUI thread.");
        ...
        if (!StringInput(title, result, maxLength, [](void* ptr,
        Result r) {
