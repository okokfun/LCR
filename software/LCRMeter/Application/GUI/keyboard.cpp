...
void Keyboard::draw(coords_t offset) {
    ...
    for (i = 1; i < LAYOUT_X; i++) {
        ...
        if (i == 3 || i == LAYOUT_X - 3)
            length += spacingY;
        display_VerticalLine(upperLeft.x + i * spacingX,
                             upperLeft.y + 1,
                             length);
    }
    ...
    if (selectedY == LAYOUT_Y && selectedX < 3) {
        ...
        display_SetForeground(Border);
    display_String(upperLeft.x + (3 * spacingX - 5 *
                                  Font->width) / 2,
                   upperLeft.y + LAYOUT_Y * spacingY + (spacingY -
                           Font->height) / 2,
                   "SHIFT");
    /* SPACE */
    if (selectedY == LAYOUT_Y && selectedX >= 3
        && selectedX < LAYOUT_X - 3) {
        /* space is selected */
        display_SetForeground(Selected);
    } else
        display_SetForeground(Border);
    display_String(
                    upperLeft.x + ((LAYOUT_X - 6) * spacingX - 5 * Font->width) /
                    2
                    + 3 * spacingX,
                    upperLeft.y + LAYOUT_Y * spacingY + (spacingY -
                            Font->height) / 2,
