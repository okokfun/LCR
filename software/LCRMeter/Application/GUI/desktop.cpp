...
void Desktop::draw(coords_t offset) {
    ...
    if (focussed == -1
        || apps[focussed]->state != App::State::Running) {
    ...
    display_RectangleFull(offset.x, offset.y,
                          offset.x + IconBarWidth - 1,
    ...
    for (i = 0; i < AppCnt; i++) {
        ...
        if (focussed == i) {
            ...
            display_SetForeground(COLOR_BLACK);
        display_VerticalLine(offset.x + 0,
                             offset.y + i * IconSpacing + 3,
                             IconSpacing - 6);
        display_VerticalLine(offset.x + 1,
                             offset.y + i * IconSpacing + 1,
                             IconSpacing - 2);
