...
void Unit::StringFromValue(char* to, uint8_t len,
                           int32_t val,
                           const Unit::unit* unit[]) {
    if (unit == Unit::Hex) {
...
bool Unit::SIStringFromFloat(char* to, uint8_t len,
                             float val, char smallest_prefix) {
    if (len < 5) {
    ...
    float postDot = abs(val) / SI_prefixes[prefix_index].factor -
                    preDot;
...
int32_t Unit::ValueFromString(const char* s,
                              uint32_t multiplier) {
    int32_t value = 0;
    int8_t sign = 1;
    uint32_t dotDivider = 0;
