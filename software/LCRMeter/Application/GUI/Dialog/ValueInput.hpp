...
class ValueInput {
  public:
    ...
    ValueInput(const char* title, T* value,
               const Unit::unit* unit[] = Unit::None,
        ...
        l = new Label((fieldSizeX + 3 * fieldIncX) / Font_Big.width,
                      Font_Big,
        ...
        c->attach(backspace, COORDS(fieldOffsetX,
                                    fieldOffsetY + 3 * fieldIncY));
        auto abort = new Button("Abort", Font_Big, [](void* ptr,
        Widget * w) {
        ...
        c->attach(abort,
                  COORDS(fieldOffsetX + 4 * fieldIncX,
                         fieldOffsetY + 3 * fieldIncY));
        auto ec = new EventCatcher(c, [](GUIEvent_t* const ev) ->
        bool {
  ...
  private:
    void CreateUnitButtons(Container* c, coords_t topLeft,
                           uint16_t incX,
    ...
    void UnitPressed(Widget* w) {
        if (unit == Unit::Hex || unit == Unit::None) {
            *value = strtol(string, nullptr, 0);
            ...
            // find correct unit
            while (*unit) {
                if (!strncmp(b->getName(), (*unit)->name,
                             strlen(b->getName()))) {
...
template <> void ValueInput<float>::CreateUnitButtons(
                Container* c, coords_t topLeft, uint16_t incX,
                uint16_t incY, uint16_t sizeX, uint16_t sizeY);
