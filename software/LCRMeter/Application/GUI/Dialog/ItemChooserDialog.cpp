...
ItemChooserDialog::ItemChooserDialog(const char* title,
                                     const char* const* items, uint8_t initial_selection,
                                     Callback cb,
                                     void* ptr) {
    this->cb = cb;
    this->ptr = ptr;
    ...
    auto i = new ItemChooser(items, &value, Font_Big, 6,
                             c->getSize().x);
    ...
    auto abort = new Button("ABORT", Font_Big,
                            ...
                            &ItemChooserDialog::ButtonPressed>::cfn, this,
                            COORDS(c->getSize().x / 2 - 10, 40));
    auto ec = new EventCatcher(i, [](GUIEvent_t* const ev) ->
    bool {
...
void ItemChooserDialog::EventCaught(Widget* w,
                                    GUIEvent_t* ev) {
    //	if (ev->type == EVENT_BUTTON_CLICKED) {
    //		bool ret = false;
