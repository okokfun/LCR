...
static void touch_SampleADC(uint16_t* rawX, uint16_t* rawY,
                            uint16_t samples) {
    uint16_t i;
    uint32_t X = 0;
    uint32_t Y = 0;
...
int8_t touch_GetCoordinates(coords_t* c) {
    ...
    c->x = constrain_int16_t(c->x * Cal.scaleX + Cal.offsetX, 0,
                             DISPLAY_WIDTH - 1);
    c->y = constrain_int16_t(c->y * Cal.scaleY + Cal.offsetY, 0,
                             DISPLAY_HEIGHT - 1);
    return ret;
}
...
static coords_t GetCalibrationPoint(bool top,
                                    coords_t cross) {
    constexpr uint16_t barHeight = 20;
    int16_t y = top ? 0 : DISPLAY_HEIGHT - Font_Big.height -
                barHeight - 1;
    ...
    do {
        if (getRaw(&c)) {
            ...
            display_VerticalLine(bar, y + Font_Big.height + 1,
                                 barHeight - 2);
        } else {
            bar = 1;
            samples = 0;
            ...
            display_RectangleFull(1, y + Font_Big.height + 1,
                                  DISPLAY_WIDTH - 2,
