...
static constexpr ad5940_pga_gain_t PGA_gain =
                AD5940_PGA_GAIN_4;
...
static Calibration
calibration_points[num_rtia_steps][ARRAY_SIZE(
        calibration_frequencies)];
static int32_t calibration_BiasVoltageOffset;

static float interpolate(float value, float scaleFromLow,
                         float scaleFromHigh,
...
static void SetADCAverages(uint32_t freq) {
    uint32_t rawSamplesPerPeriod =
                    HardwareLimits::ADCSampleRate / freq;
...
static Calibration GetCalibration(ad5940_hsrtia_t rtia,
                                  uint32_t freq) {
    // find previous and next calibration point in frequency
    uint16_t i;
    for (i = 0; i < ARRAY_SIZE(calibration_frequencies) - 1;
         i++) {
    ...
    c.MagCal = interpolate(freq, calibration_frequencies[i],
                           calibration_frequencies[i + 1], CalLower.MagCal,
                           CalHigher.MagCal);
    c.PhaseCal = interpolate(freq, calibration_frequencies[i],
                             calibration_frequencies[i + 1], CalLower.PhaseCal,
                             CalHigher.PhaseCal);
    return c;
}

static uint32_t GetCalibrationExcitationAmplitude(
...
static bool SetBias(int32_t biasVoltage,
    ...
    uint16_t code_DAC = util_Map(V_DAC, 200000, 2400000, 0,
                                 4095);
    ad5940_take_mutex(&ad);
    ad5940_modify_reg(&ad, AD5940_REG_LPDACDAT0, code_DAC,
                      0x00000FFF);
    ad5940_release_mutex(&ad);
...
static void SetSwitchesForRCAL() {
    ad5940_take_mutex(&ad);
    ad5940_modify_reg(&ad, AD5940_REG_SWCON,
...
static void SetCalibrationMeasurement(uint32_t freq,
                                      ad5940_hsrtia_t rtia) {
    SetSwitchesForRCAL();
    ...
    wave.sine.amplitude = GetCalibrationExcitationAmplitude(
                                          rtia);
...
static void RunBiasVoltageCalibration() {
    ...
    ad5940_set_ADC_mux(&ad, AD5940_ADC_MUXP_VBIAS_CAP,
            ...
            case State::Measuring: {
                    ...
                    if (sampleCnt > settings.averages) {
                        if (voltageMeasurement) {
                            ...
                            ad5940_take_mutex(&ad);
                            bool voltageMeasurementClipped = ad5940_read_reg(&ad,
                                                             AD5940_REG_INTCFLAG0) & 0x30;
                            ad5940_release_mutex(&ad);
                            constexpr uint32_t ADC_max_value =
                                            32768		// 16384 is the value according to datasheet, ADC actually reports values up to 32768
                            ...
                            if (state == State::Measuring) {
                                 ...
                                 */
                                result.Z = std::complex<float>(mag * cos(phase),
                                                               mag * sin(phase));
                                result.frequency = settings.frequency;
                                constexpr float mag_to_RMS = (1UL <<
                                                              17)		// compensate division in ad5940_get_dft_result
                                                             * 1.835f / 32768		// ADC bits and slope compensation
                                                             * 0.25f 			// Compensate additional 2 bits in DFT
                                                             * M_SQRT1_2;		// Peak to RMS
                                result.RMS_U = sumMagVoltage * mag_to_RMS * 10 /
                                               ad5940_PGA_gain_to_value10(PGA_gain);
                                result.RMS_I = sumMagCurrent * mag_to_RMS * 10 /
                                               ad5940_PGA_gain_to_value10(PGA_gain);
                                /*
...
bool Frontend::Init() {
    ...
    Persistence::Add(calibration_points,
                     sizeof(calibration_points));
    Persistence::Add(&calibration_BiasVoltageOffset,
                     sizeof(calibration_BiasVoltageOffset));
    // Set calibration to default values in case of missing persistence data
    for (uint8_t i = 0; i < AD5940_HSRTIA_OPEN; i++) {
        for (uint8_t j = 0; j < ARRAY_SIZE(calibration_frequencies);
             j++) {
     ...
     */
    ad5940_write_reg(&ad, AD5940_REG_ADCMIN,
                     HardwareLimits::ADCHeadroom);
    ad5940_write_reg(&ad, AD5940_REG_ADCMINSM,
                     UINT16_MAX - HardwareLimits::ADCHeadroom);
    ad5940_write_reg(&ad, AD5940_REG_ADCMAX,
                     UINT16_MAX - HardwareLimits::ADCHeadroom);
    ad5940_write_reg(&ad, AD5940_REG_ADCMAXSMEN,
                     UINT16_MAX - HardwareLimits::ADCHeadroom);
    ...
    ad5940_set_HSTIA(&ad, AD5940_HSRTIA_200, 0,
                     AD5940_HSTIA_VBIAS_1V1, true,
                     AD5940_HSTSW_RCAL1);
    ...
    queueHandle = xQueueGenericCreateStatic(msgQueueLen,
                                            sizeof(Message),
                                            queueBuf, &msgQueue, 0);
    // Start task
