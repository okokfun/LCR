...
uint8_t* USBD_FS_ProductStrDescriptor(USBD_SpeedTypeDef
...
#endif
__ALIGN_BEGIN uint8_t
USBD_StringSerial[USB_SIZ_STRING_SERIAL] __ALIGN_END = {
...
uint8_t* USBD_FS_ProductStrDescriptor(USBD_SpeedTypeDef
                                      speed, uint16_t* length) {
    if (speed == 0)
        USBD_GetString((uint8_t*)USBD_PRODUCT_STRING_FS,
                       USBD_StrDesc, length);
    else
        USBD_GetString((uint8_t*)USBD_PRODUCT_STRING_FS,
                       USBD_StrDesc, length);
...
uint8_t* USBD_FS_ConfigStrDescriptor(USBD_SpeedTypeDef speed,
    ...
    if (speed == USBD_SPEED_HIGH)
        USBD_GetString((uint8_t*)USBD_CONFIGURATION_STRING_FS,
                       USBD_StrDesc, length);
    else
        USBD_GetString((uint8_t*)USBD_CONFIGURATION_STRING_FS,
                       USBD_StrDesc, length);
...
uint8_t* USBD_FS_InterfaceStrDescriptor(USBD_SpeedTypeDef
                                        speed, uint16_t* length) {
    if (speed == 0)
        USBD_GetString((uint8_t*)USBD_INTERFACE_STRING_FS,
                       USBD_StrDesc, length);
    else
        USBD_GetString((uint8_t*)USBD_INTERFACE_STRING_FS,
                       USBD_StrDesc, length);
